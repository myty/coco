 ...
 *
 * Requirements:
 *  - `deno task compile` or `deno compile` must produce `bin/lomux`
...
Deno.test({
  name: "daemon — lomux start binds to 127.0.0.1:11434 and /health returns 200",
  ignore: true, // TODO: enable after T014/T016/T017 complete
  async fn() {
    const stat = await Deno.stat("bin/lomux").catch(() => null);
    if (!stat) {
    ...
    const proc = new Deno.Command("bin/lomux", {
      args: ["start"],
      stdout: "piped",
      stderr: "piped",
    }).spawn();
    ...
    const stop = await new Deno.Command("bin/lomux", {
      args: ["stop"],
      stdout: "piped",
    }).output();
    const stopText = new TextDecoder().decode(stop.stdout).trim();
    assertEquals(stopText, "Lomux stopped.");
...
Deno.test({
  name: "daemon — lomux start prints 'already running' when running",
  ignore: true, // TODO: enable after T014/T016/T017 complete
  async fn() {
    const proc = new Deno.Command("bin/lomux", {
      args: ["start"],
    ...
    const second = await new Deno.Command("bin/lomux", {
      args: ["start"],
      stdout: "piped",
    }).output();
    ...
    await new Deno.Command("bin/lomux", { args: ["stop"] }).output();
    proc.unref();
  },
});

Deno.test({
  name: "daemon — lomux stop when not running prints 'not running'",
  ignore: true, // TODO: enable after T014/T016/T017 complete
  async fn() {
    const out = await new Deno.Command("bin/lomux", {
      args: ["stop"],
      stdout: "piped",
    }).output();
    const text = new TextDecoder().decode(out.stdout).trim();
    assertEquals(text, "Lomux is not running.");
  },
});
