# lomux Logo Brief

## Objective
Create a production-ready logo system for lomux that communicates a local AI gateway and routing function.

## Brand Direction
- Product name: lomux (lowercase only)
- Personality: practical, calm, technical, trustworthy
- Core idea: local routing, multiplexing, control point between coding agents and GitHub Copilot

## Symbol Ideas
- Signal split or merge shape
- Route node or gateway icon
- M + path geometry with strong silhouette

## Visual Constraints
- Must be legible at favicon sizes (16x16 and 32x32)
- Must work on light and dark backgrounds
- Must have monochrome version
- Avoid visual complexity and thin details that disappear when scaled down

## Deliverables
- Primary logo lockup (icon + wordmark)
- Icon-only mark
- Monochrome variants
- Favicon exports: 16x16, 32x32, 48x48
- Web exports: SVG and PNG at 1x, 2x, and 3x
- Source files: editable vector format

## Acceptance Checklist
- The wordmark reads clearly as lomux at small sizes
- The icon is distinct from common generic cloud logos
- Contrast passes WCAG AA against site background colors
- Assets can replace current site logo references without layout breakage
